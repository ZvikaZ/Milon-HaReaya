...
from docx_fork_ludoo.enum.text import WD_BREAK
from docx_fork_ludoo.opc.constants import CONTENT_TYPE as CT, RELATIONSHIP_TYPE as RT
from docx_fork_ludoo.package import Package
...
from docx_fork_ludoo.shared import lazyproperty


_thisdir = os.path.split(__file__)[0]
_default_docx_fork_ludoo_path = os.path.join(_thisdir, 'templates', 'default.docx_fork_ludoo')


class Document(object):
    """
    Return a |Document| instance loaded from *docx_fork_ludoo*, where *docx_fork_ludoo* can be
    either a path to a ``.docx_fork_ludoo`` file (a string) or a file-like object. If
    *docx_fork_ludoo* is missing or ``None``, the built-in default document "template"
    is loaded.
    """
    def __init__(self, docx_fork_ludoo=None):
        super(Document, self).__init__()
        document_part, package = self._open(docx_fork_ludoo)
    ...
    @staticmethod
    def _open(docx_fork_ludoo):
        """
        Return a (document_part, package) 2-tuple loaded from *docx_fork_ludoo*, where
        *docx_fork_ludoo* can be either a path to a ``.docx_fork_ludoo`` file (a string) or a
        file-like object. If *docx_fork_ludoo* is ``None``, the built-in default
        document "template" is loaded.
        """
        docx_fork_ludoo = _default_docx_fork_ludoo_path if docx_fork_ludoo is None else docx_fork_ludoo
        package = Package.open(docx_fork_ludoo)
        document_part = package.main_document
        if document_part.content_type != CT.WML_DOCUMENT_MAIN:
            tmpl = "file '%s' is not a Word file, content type is '%s'"
            raise ValueError(tmpl % (docx_fork_ludoo, document_part.content_type))
