...
% Move page numbers outward without extending the header line
% Use \hspace* with negative values to shift content, not \fancyheadoffset which extends the box
\newcommand{\fancyheadlo}{\hspace*{-0.4cm}\mynormalsize{\PolythumbWrapperLeft\hspace{-1mm}\thepage}}
\newcommand{\fancyheadre}{\mynormalsize{\thepage\PolythumbWrapperRight}\hspace*{-0.5cm}}
...
% Control column balancing tolerance
% Problem: At \end{multicols}, LaTeX tries very hard to balance columns equally.
% With default columnbadness=10000, it accepts extremely underfull columns,
% creating ugly spacing by over-stretching glue between paragraphs/entries.
% Solution: Lower value rejects overly-stretched solutions, preferring unbalanced
% columns over ugly spacing. Range: 0 (strict) to 10000 (tolerant).
% Current value (1000) works with reduced stretch in \parskip and \paragraphs below.
% If too much stretching: lower to 500. If too many unbalanced columns: raise to 2000.
...
% Control vertical spacing stretch for column balancing
% Without the plus/minus flexibility here and in \paragraphs, we get unbalanced columns.
% Reduced stretch from 1pt to 0.5pt to limit ugly spacing when balancing.
%
% How this works together with columnbadness above:
% 1. Less stretch available → Balancing requires higher badness sooner
% 2. columnbadness=1000 → Rejects solutions with badness > 1000
% 3. Result: multicol gives up on balancing more often, producing unbalanced columns
%    instead of ugly spacing
%
% Tuning guide:
% - If still too much stretching: reduce to 0.25pt and 0.025ex, or lower columnbadness to 500
% - If too many unbalanced columns: increase to 0.75pt and 0.075ex, or raise columnbadness to 2000
% - Note: "plus 0.5pt" doesn't mean "max 0.5pt stretch" - TeX can stretch infinitely,
%   but higher stretch = higher badness, which columnbadness will eventually reject
\setlength{\parskip}{0pt plus 0.5pt}

\newcommand{\paragraphs}{
    \par\vspace{1ex plus 0.05ex minus 0.05ex}
}
...
\newcommand{\תהגדרהמודגשת}[1]{\textbf{\תהגדרה{#1}}}
